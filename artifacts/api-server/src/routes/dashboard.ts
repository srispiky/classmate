...
import { db, studentsTable, coursesTable, assignmentsTable, assessmentsTable, activityTable } from "@workspace/db";
import {
...
router.get("/dashboard/summary", async (_req, res): Promise<void> => {
  ...
  const completionRate = assignments.length > 0 ? completed / assignments.length : 0;

  // at-risk: students with avg score < 60
  ...
  for (const student of students) {
    ...
    if (avg >= 80) topPerformers.push({ id: student.id, name: student.name, averageScore: Math.round(avg) });
  }
...
router.get("/dashboard/grade-breakdown", async (_req, res): Promise<void> => {
  const courses = await db.select().from(coursesTable);
  const assessments = await db.select().from(assessmentsTable);
