...
router.post("/students", async (req, res): Promise<void> => {
  ...
  res.status(201).json(GetStudentResponse.parse({ ...student, avatarUrl: student.avatarUrl ?? null, createdAt: student.createdAt.toISOString() }));
});
...
router.get("/students/:id", async (req, res): Promise<void> => {
  ...
  res.json(GetStudentResponse.parse({ ...student, avatarUrl: student.avatarUrl ?? null, createdAt: student.createdAt.toISOString() }));
});
...
router.patch("/students/:id", async (req, res): Promise<void> => {
  ...
  res.json(UpdateStudentResponse.parse({ ...student, avatarUrl: student.avatarUrl ?? null, createdAt: student.createdAt.toISOString() }));
});
...
router.get("/students/:id/progress", async (req, res): Promise<void> => {
  ...
  const assignments = await db.select().from(assignmentsTable).where(eq(assignmentsTable.studentId, studentId));
  const assessments = await db.select().from(assessmentsTable).where(eq(assessmentsTable.studentId, studentId));
  ...
  const avgScore = gradedAssignments.length > 0
    ? gradedAssignments.reduce((sum, a) => sum + ((a.score ?? 0) / a.maxScore) * 100, 0) / gradedAssignments.length
    : 0;
  ...
  const progress = {
    ...
    completionRate: assignments.length > 0 ? Math.round((completed / assignments.length) * 100) / 100 : 0,
